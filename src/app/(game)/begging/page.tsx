...
import { Hand, Zap, Coins, AlertTriangle, Package } from "lucide-react";
...
export default function BeggingPage() {
  ...
  const [lastResult, setLastResult] = useState<BegResult | null>(null);
  const [error, setError] = useState("");
  const [sessionCount, setSessionCount] = useState(0);

  useEffect(() => {
    document.title = "Begging — TribalMMO";
  }, []);
  ...
  const beggingSkill = character.skills?.find((s) => s.name === "Begging");
  const currentLevel = beggingSkill?.level || 1;
  const xp = beggingSkill?.experience || 0;
  ...
  const doBeg = async () => {
    ...
    const { data, error: rpcError } = await supabase.rpc("beg", {
    ...
    const result = data as unknown as BegResult;
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      {lastResult && (
        <div className="card animate-fade-in" style={{
          ...
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-sm font-bold font-heading" style={{ color: lastResult.success ? "#c9a84c" : "#b83a3a" }}>
              {lastResult.success ? "Charity Received!" : "Turned Away"}
            </h2>
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex items-center gap-2 px-3 py-2 rounded bg-slate-900/40 border border-slate-800/30">
              ...
              <span className="text-slate-300 text-sm font-semibold">+{lastResult.xp_gained} Begging XP</span>
            </div>
            {lastResult.success && lastResult.gold > 0 && (
              <div className="flex items-center gap-2 px-3 py-2 rounded bg-slate-900/40 border border-slate-800/30">
                <Coins size={14} className="text-[#c9a84c]" />
                <span className="text-[#c9a84c] text-sm font-semibold">+{lastResult.gold} gold</span>
              </div>
            )}
      ...
      <div className="forge-card">
        <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Begging Skill</h2>
        <div className="flex items-center justify-between mb-2">
        ...
        <div className="w-full h-2 bg-slate-900 rounded-full overflow-hidden border border-slate-800/20">
          <div className="h-full rounded-full transition-all duration-500" style={{ width: `${xpPercent}%`, background: "#c9a84c80" }} />
        </div>
      ...
      <Button
        ...
        icon={<Hand size={18} />}
        onClick={doBeg}
        disabled={begging || character.computed_stamina < staminaCost}
        loading={begging}
      >
        {character.computed_stamina < staminaCost
          ? `Not Enough Stamina (need ${staminaCost})`
          : `Beg for Charity (-${staminaCost} stamina)`}
      </Button>
      <p className="text-slate-600 text-xs text-center -mt-3">
        Costs {staminaCost} stamina per attempt &middot; Speed improves success chance
      </p>
      ...
      <div className="forge-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div className="flex items-center gap-2 p-2 rounded bg-slate-900/40 border border-slate-800/20">
            <Package size={14} className="text-slate-400" />
            <div>
              <div className="text-slate-300 text-xs font-semibold">Gathering (per 10 levels)</div>
              <div className="text-slate-600 text-[10px]">+1 bonus gold per successful beg</div>
            </div>
          </div>
          <div className="flex items-center gap-2 p-2 rounded bg-slate-900/40 border border-slate-800/20">
            <Zap size={14} className="text-slate-400" />
            <div>
              <div className="text-slate-300 text-xs font-semibold">Equipment Speed</div>
              <div className="text-slate-600 text-[10px]">Improves success chance slightly</div>
            </div>
          </div>
        </div>
      </div>
