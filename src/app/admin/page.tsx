...
export default function AdminDashboard() {
  const [stats, setStats] = useState<StatCard[]>([
    ...
    { label: "Clans", value: 0, icon: Shield, color: "text-slate-300" },
  ...
  const fetchStats = async () => {
    ...
    setStats([
      ...
      { label: "Clans", value: clans.count ?? 0, icon: Shield, color: "text-slate-300" },
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-5xl">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-0.5">TribalMMO server overview</p>
      </div>
      ...
      <div className="bg-[#1a181e] border border-[#262328] rounded-xl p-5">
        <h2 className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-4">Recent Characters</h2>
            ...
            {recentCharacters.map((c, i) => (
              <div key={i} className="flex items-center justify-between py-2 px-3 rounded-lg bg-[#1e1c22]">
                <div>
                  ...
                  <span className="text-slate-500 text-sm ml-2">{c.background}</span>
                </div>
